...
void ezled_print(const char *pstr){
  ...
  ezled_send_cmd(0x01, CMD_PRINT, pfirst, len_str);
  pstr += len_str;
  len_str = ezled_cast_str(pstr);
  if(len_str)
    ezled_send_cmd(0x02, CMD_PRINT, pstr, len_str);
  else
...
void ezled_hightlight(uint16_t which){
  ...
  if(which<5){
    buff[0] = which;
    ezled_send_cmd(0x01, CMD_SET_HLIGHT, buff, 1);
    buff[0] = LED_DISPLAY_BIT;
    ezled_send_cmd(0x02, CMD_SET_HLIGHT, buff, 1);
  }
  else if(which<10){
    which -= 5;
    buff[0] = which;
...
void ezled_set_blink(uint8_t which){
  ...
  if(which < LED_DISPLAY_BIT){
    data = 1<<which;
    ezled_send_cmd(0x01, CMD_SETBLINK, &data, 1);
    data = 0;
    ezled_send_cmd(0x02, CMD_SETBLINK, &data, 1);
  }
  else{
    data = 0;
    ezled_send_cmd(0x01, CMD_SETBLINK, &data, 1);
...
void ezled_set_contrast(uint8_t contrast_a, uint8_t contrast_b){
  ...
  buff[1] = contrast_a;
  ezled_send_cmd(0x01, CMD_SETCONTRASTA, buff, 2);
  ezled_send_cmd(0x02, CMD_SETCONTRASTA, buff, 2);

  buff[1] = contrast_b;
  ezled_send_cmd(0x01, CMD_SETCONTRASTB, buff, 2);
  ezled_send_cmd(0x02, CMD_SETCONTRASTB, buff, 2);
}
...
void ezled_set_hlight_contrast(uint8_t contrast_h, uint8_t contrast_l){
  ...
  ezled_send_cmd(0x01, CMD_SETCONTRASTC, buff, LED_DISPLAY_BIT);
  ezled_send_cmd(0x02, CMD_SETCONTRASTC, buff, LED_DISPLAY_BIT);
}

void ezled_set_blink_speed(blink_speed_def speed){
  char data;
  data = speed;
  ezled_send_cmd(0x01, CMD_SETBLINK_SPEED, &data, 1);
  ezled_send_cmd(0x02, CMD_SETBLINK_SPEED, &data, 1);
}
...
void ezled_set_global_contrast(uint8_t contrast){
  ezled_set_contrast(contrast, 0x00);
  ezled_set_hlight_contrast(contrast, 0x10);
}
